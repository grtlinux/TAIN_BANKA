package ic.vela.ibridge.serverbatch.hwi51.client;

import ic.vela.ibridge.serverbatch.hwi51.common.BatchChkFile;
import ic.vela.ibridge.serverbatch.hwi51.common.BatchFile;
import ic.vela.ibridge.serverbatch.hwi51.common.BatchFolder;
...
public class BatchClientMain
{
	...
	/*------------------------------------------------------------------*/
	{
		...
		if (flag) {
			...
			if (flag) {
				 ...
				 * hashMapFolder를 생성한다.
				 */
				hashMapFolder = new HashMap<String,BatchFolder> (5, 5);
				...
				for (int i=1; i <= MAX_FOLDER_CNT; i++) {
					...
					if (strSrcFolderName != null) {
						 ...
						 * 저장객체를 생성한다.
						 */
						BatchFolder folder = new BatchFolder(strKey, strSrcFolderName, strTgtFolderName);
	 ...
	 */
	/*------------------------------------------------------------------*/
	private boolean doConnectAndSendData(File fileFolder, File file, BatchFile infoBatchFile)
	{
		boolean flag = true;
	 ...
	 */
	/*------------------------------------------------------------------*/
	private void doPostProcess(File fileFolder, File file, BatchFile infoBatchFile)
	{
		boolean flag = true;
			...
			try {
				...
				while (true) {
					...
					while (iterKeyFolder.hasNext()) {
						...
						BatchFolder batchFolder = hashMapFolder.get(keyFolder);
								...
								new FileFilter() {
									public boolean accept(File file) {
										...
										if (flag) {
											...
											if (!flag) {   // TODO : 테스트용 20130515000000
											...
											if (strCompareDate.compareTo(strLastModifiedDate) <= 0) {
												...
												BatchFile batchFile = hashMapFile.get(strKey);
												if (batchFile != null) {
													/*
													 * 이 파일은 해당일에도 속하고
													 * 배치파일 목록에도 포함된 파일이다.
						...
						for (int i=0; i < files.length; i++) {
							...
							if (files[i].isFile()) {
								 ...
								 */
								BatchChkFile obj = (BatchChkFile) hashMapChkFile.get(strKey);
								if (obj == null) {
									/*
									 * 배치처리를 한다.
									 */

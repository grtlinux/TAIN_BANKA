	...
	/*------------------------------------------------------------------*/
	{
		...
		if (flag) {     // DATE.2013.06.22
			try {
				 ...
				 * 관리대상의 리소스 큐를 얻는다.
				 */
				this.writeQueue = this.extManager.getResourceBanker().getQueueBanker().getQueue().get("SEND_RECVFROMIB_QUEUE");
				...
				while (!errFlag) {
					...
					if (strReqStreamXml != null) {
						...
						if (flag) {
							 ...
							 * Ext의 정보를 구한다.
							 */
							strListenName = this.listen.getStrListenName();
							iListenPort = this.listen.getiListenPort();
						}
						
						if (flag) Logger.log("[REQ -> RES] : [%s:%d][%s]", strListenName, iListenPort, strReqStreamXml);

...
/*------------------------------------------------------------------*/
{
			...
			try {
				...
				if (flag) {
					...
					for (int i=1; i < 100; i++) {
						...
						String strConnServerCode  = prop.getProperty("system.ib.param.host." + strIndex + ".server.code");
						...
						if (strConnName != null) {
							 ...
							 * null 이 아니면 저장한다.
							 */
							SocketHashMapConnect connect = new SocketHashMapConnect(strConnName, strConnServerCode, strConnFepId, strConnHostIp, strConnHostPort, strPoll);
					...
					if (!flag) {
						...
						while (iterKeys.hasNext()) {
							...
							if (flag) {
								...
								System.out.println("[" + key + "] [strConnServerCode =" + listen.getStrConnServerCode() + "]");
						...
						try {
							...
							if (!flag) {
								...
								if (flag) Logger.log("getKeepAlive=%s, getTcpNoDelay=%s, getSoTimeout=%d, getSoLinger=%d, getReuseAddress=%s"
										, socket.getKeepAlive(), socket.getTcpNoDelay(), socket.getSoTimeout(), socket.getSoLinger(), socket.getReuseAddress());
								 ...
								 * TODO DATE.2013.08.10 : 아래 이유로 접속시도는 생성되는 Thread, AttachObject에서 처리하기로 한다.
	...
	/*------------------------------------------------------------------*/
	@SuppressWarnings("unused")
	private void tryMultiConnection2()
	/*------------------------------------------------------------------*/
	{
		boolean flag = true;
		...
		if (flag) {    // DATE.2013.06.29
			...
			if (!flag) Logger.log("in tryMultiConnection2()");
		...
		if (flag) {     // DATE.2013.06.23
			...
			if (strReqStreamXml != null) {
				...
				if ("POLL_OUT_KILL_ME".equalsIgnoreCase(strReqStreamXml)) {
					/*
					 * TODO DATE.2013.07.16 : 죽여달라는 메시지를 받으면 과감하게 죽여준다.ㅋㅋㅋㅋㅋ
					 */

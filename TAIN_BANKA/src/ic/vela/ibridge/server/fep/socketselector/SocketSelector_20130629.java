...
/*------------------------------------------------------------------*/
{
				...
				if (flag) {
					...
					for (int i=1; i < 100; i++) {
						...
						if (strConnName != null) {
							 ...
							 * null 이 아니면 저장한다.
							 */
							SocketHashMapConnect connect = new SocketHashMapConnect(strConnName, "", strConnFepId, strConnHostIp, strConnHostPort, "");
	...
	/*------------------------------------------------------------------*/
	{
		...
		if (flag) {
			...
			SocketChannelAttachObject attachObject = new SocketChannelAttachObject(null);  // ERROR
			attachObject.start();
	...
	/*------------------------------------------------------------------*/
	{
		...
		if (flag) {   // TODO DATE.2013.06.29
			
			while (!errFlag) {
				/*
				 * errFlag = false 이므로 프로세싱을 시작한다.
				 */
				for (int i=0;!errFlag; i=(i+1) % 100) {

package ic.vela.ibridge.server.fep.sendtoap;
 ...
 */
/*==================================================================*/
public class SendToAp extends Thread 
/*------------------------------------------------------------------*/
{
	/*==================================================================*/
	/*
	...
	public SendToAp()
	{
		...
		if (flag) {     // DATE.2013.06.22
			try {
				...
				this.readQueue  = this.fepManager.getResourceBanker().getQueueBanker().getQueue().get("RECV_SENDTOAP_QUEUE");
				
				// SendToAp -> ServerSocketSelector
				this.writeQueue = this.fepManager.getResourceBanker().getQueueBanker().getQueue().get("RECV_SERVERSELECTOR_QUEUE");
				
				/*
				 * 전문저장 객체를 얻는다.
				 */
				//this.hashMapXml = this.fepManager.getResourceBanker().getHashMapBanker().getHashMap().get("XMLOBJECT_HASHMAP");
				this.hashMapXml = new HashMap<String,XmlObject> (5, 5);  // used in only local
				
				this.hashMapQueue = (HashMap<String,IBridgeQueue>) this.fepManager.getResourceBanker().getHashMapBanker().getHashMap().get("IBRIDGEQUEUE_HASHMAP");
		...
		if (flag) {    // TODO DATE.2013.06.25
			...
			while (true) {
				...
				if (flag) {
					...
					object = (Object) this.readQueue.get(100);  // 1초 멈칫한다.
					if (object == null) {
						continue;
					}
				}

...
/*------------------------------------------------------------------*/
{
	...
	/*------------------------------------------------------------------*/
	{
		...
		if (flag) {     // TODO DATE.2013.06.30 : ServerSocketHashMapListen listen 만 사용
			try {
				/*
				 * 객체의 이름을 설정한다.
				 */
				 ...
				 * 관리대상의 리소스 큐를 얻는다.
				 */
				this.writeQueue = this.apManager.getResourceBanker().getQueueBanker().getQueue().get("SEND_RECVFROMIB_QUEUE");
				this.readQueue  = this.apManager.getResourceBanker().getQueueBanker().getQueue().get("RECV_SERVERSELECTOR_QUEUE");
				 ...
				 * SendToIB 에 넘긴다.
				 */
				this.sendToApQueue  = this.apManager.getResourceBanker().getQueueBanker().getQueue().get("RECV_SENDTOIB_QUEUE");
